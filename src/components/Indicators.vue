  ...
  <div class="summary-card">
    ...
    <div class="chart-navigation">
      ...
      <div class="chart-area-wrapper">
        <!-- Плавающая обводка -->
        <div class="focused-border" :style="focusedBorderStyle" />

        <div class="chart-area" :style="trackStyle">
          <div
            v-for="(day, dayIndex) in allDates"
            :key="dayIndex"
            class="chart-column"
            :class="{ 'active-day': hoveredDay === dayIndex }"
          >
            <div class="date__line"></div>
            <p class="date">{{ day.label }}</p>

            <div class="bar-group">
              <div v-for="(value, barIndex) in day.values" :key="barIndex" class="mini-bar-group">
                <div class="mini-bar-shadow"></div>
                <div class="mini-bar-wrapper">
                  <div
                    class="mini-bar"
                    :style="{ height: value + 'px' }"
                    :class="{
                      active: hoveredDay === dayIndex && hoveredBar === barIndex,
                      'first-bar': barIndex === 0,
                    }"
                    @mouseenter="
                      () => {
                        hoveredDay = dayIndex
...
const focusedBorderStyle = computed(() => {
  ...
  const localIndex = (() => {
    // если остаётся меньше range элементов — сместим обводку вправо
    if (startIndex.value + visibleItems >= allDates.value.length) {
      const lastVisibleIndex = allDates.value.length - startIndex.value - 1
      return Math.min(focusedIndex.value - startIndex.value, lastVisibleIndex)
    }
    return focusedIndex.value - startIndex.value
  })()
...
.date__line::after {
  ...
  z-index: -11; Added z-index to ensure it appears above other elements */
}
.tooltip {
  position: absolute;
  bottom: 100%;
...
.mini-bar-group:hover .mini-bar-shadow {
  background: linear-gradient(
    to bottom,
...
.mini-bar-group:hover .mini-bar {
  /* filter: brightness(0.9); */
}
.mini-bar-group:hover .mini-value {
  color: var(--greentext-color);
}
</style>
