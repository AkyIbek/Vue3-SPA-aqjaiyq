...
// --- items с ключами и переводимыми метками
const typeItems = computed(() => [
  { key: 'all', label: t('services.filter.item-list.item-6') }, // Показать все
  { key: 'online', label: t('services.filter.item-list.item-1') }, // онлайн
  { key: 'offline', label: t('services.filter.item-list.item-8') }, // офлайн
])

const regionItems = computed(() => [
  { key: 'all', label: t('services.filter.item-list.item-1') }, // Все регионы
...
// --- options для CustomSelect (только метки)
...
// --- при смене языка: подставляем соответствующую метку для текущего ключа
watch(
  typeItems,
  (newItems) => {
...
// --- когда пользователь выбирает метку в CustomSelect — находим её ключ
...
// --- данные сервисов (title/subtitle — переводимые строки, type/region — ключи)
...
// --- фильтрация по ключам (all = все)
const filteredServices = computed(() =>
  services.value.filter((s) => {
    const typeMatch = selectedTypeKey.value === 'all' || s.type === selectedTypeKey.value
    const regionMatch = selectedRegionKey.value === 'all' || s.region === selectedRegionKey.value
