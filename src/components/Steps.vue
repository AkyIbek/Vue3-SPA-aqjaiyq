<template>
  <div class="step-card">
    <div class="step__title">{{ $t('steps.item-list.item-1') }}</div>
    <div class="step__subtitle">{{ $t('steps.item-list.item-2') }}</div>
    <!-- Шаги и линия -->
    <div class="steps">
      <!-- Линия -->
      <div class="line"></div>
      ...
      <!-- Обводка, которая двигается -->
      <div
        class="active-indicator"
        :style="{ left: `${(currentStep / (steps.length - 1)) * 100}%` }"
      ></div>

      <!-- Сам кружок с числом, всегда по центру -->
    </div>
...
/* Активный круг (по центру, статичен) */
.active-circle {
  position: absolute;
  top: 50%;
  left: 50%;
